...
const assistantArray = [{ name: 'skeleton', id: 'asst_3aXwBiUZFqzcoSLUdGeFLGiG' }]
...
export default function Home() {
	...
	const [assistantId, setAssistantId] = useState<string | null>(assistantArray[0].id)
	...
	const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		setMessages((currentMessages) => [...currentMessages, { role: 'user', content: inputContent }])
		...
		try {
			...
			setMessages((currentMessages) => [...currentMessages, responseData.messages[0]])
		} catch (error) {
	...
	return (
		<div className="flex flex-col h-screen w-screen items-center justify-between gap-4 p-3">
			<div className="flex flex-col items-start gap-4 overflow-scroll">
				<h1 className="p-10 text-xl self-center">Let&apos;s get some info for your Skeleton</h1>
